...
> extends BaseDomain<TheTypesOfEvents> {
  ...
  setTitle(title: string): void {
    throw new Error("请实现 setTitle 方法" + title);
  }
  ...
  /** 复制文本到粘贴板 */
  copy(text: string) {
    throw new Error("请实现 copy 方法" + text);
  }
  getComputedStyle(el: unknown): Record<string, unknown> {
    throw new Error("请实现 getComputedStyle 方法" + el);
  }
  /** 发送推送 */
  notify(msg: { title: string; body: string }) {
    console.log("请实现 notify 方法", msg);
  }
  disablePointer() {
